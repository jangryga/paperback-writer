...
export interface CanvasConfigType {
  ...
  tabSize: number;
  stylesConfig: {
    styles: Record<keyof typeof TokenCategory | "BgColor" | "BgHighlightColor", string>;
  };
}
...
function useCanvasManager(initialCanvasContext: CanvasContextType): {
  ...
  const [context, dispatch] = useReducer((state: CanvasContextType, action: CanvasActionType) => {
    switch (action.type) {
      case "__dev_UPDATE": {
        // SAVE_SELECTION
        let preRerenderSelection = saveSelectionInternal(action.payload.element);
        // SET
      ...
      case "SAVE_SELECTION": {
        const preRerenderSelection = saveSelectionInternal(action.payload.element);
      ...
      case "RESTORE_SELECTION": {
        const element = action.payload.element;
        element.innerHTML = ReactDOMServer.renderToString(<>{state.grid.rows.map((row) => row.elements)}</>);
  ...
  const saveSelection = useCallback((element: HTMLDivElement, updateHighlightRow: boolean) => {
    dispatch({
      type: "SAVE_SELECTION",
...
export const CanvasProvider = ({
  ...
  children: ReactNode;
}) => <CanvasContext.Provider value={useCanvasManager(initialContext)}>{children}</CanvasContext.Provider>;
...
export const __dev__useUpdateState = (): UseCanvasManagerResult["__dev__updateState"] => {
